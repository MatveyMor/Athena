...
def boundary_attack(net, initial_image, target_image, verbose=1, threshold=None, max_iter=None, mu=3e-4):
    if verbose > 0:
        print('Start boundary attack')

    predict_init = net(torch.tensor(initial_image.reshape(1, 1, 28, 28)).to('cuda:0')).to('cpu').data.numpy()
    predict_target = net(torch.tensor(target_image.reshape(1, 1, 28, 28)).to('cuda:0')).to('cpu').data.numpy()
    ...
    while True:
        ...
        prediction = net(torch.tensor(trial_sample.reshape(1, 1, 28, 28)).to('cuda:0')).to('cpu').data.numpy()
        n_calls += 1
    ...
    while True:
        ...
        while True:
            ...
            for i in np.arange(10):
                ort_pert = orthogonal_perturbation(delta, adversarial_sample, target_image)
                trial_sample = adversarial_sample + ort_pert
                trial_samples.append(trial_sample)
                ort_perts.append(ort_pert)
                
            trial_samples = np.array(trial_samples)
            predictions = net(torch.tensor(trial_samples.reshape(10, 1, 28, 28)).to(dtype=torch.float).to('cuda:0')).to('cpu').data.numpy()
            n_calls += 10
            ...
            if d_score > 0.0:
                ...
                if not adversarial_momentum is None:
                    adversarial_sample += mu * adversarial_momentum
                
                pred_momentum = np.argmax(net(torch.tensor(adversarial_sample.reshape(1, 1, 28, 28)).to(dtype=torch.float).to('cuda:0')).to('cpu').data.numpy(), axis=1)
        ...
        while True:
            e_step += 1
            trial_sample = adversarial_sample + forward_perturbation(epsilon * np.linalg.norm(adversarial_sample - target_image, ord=2), adversarial_sample, target_image)
            prediction = net(torch.tensor(trial_sample.reshape(1, 1, 28, 28)).to('cuda:0', dtype=torch.float)).to('cpu').data.numpy()
            n_calls += 1
            if np.argmax(prediction) == attack_class:
                adversarial_sample = trial_sample
                epsilon /= 0.5
        ...
        prob_step = np.max(softmax(net(torch.tensor(adversarial_last.reshape(1, 1, 28, 28)).to('cuda:0', dtype=torch.float)).to('cpu').data.numpy()))
